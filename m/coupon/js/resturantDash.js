const addItem = () => {
    ...
    // Storage
    const ref = storage.ref('Glamaryid');
    ...
    auth.onAuthStateChanged((res) => {
        db.collection("items").where("key", "==", res.uid).get().then((querySnapshot) => {
            if (querySnapshot.empty) {
                resItem.innerHTML = `<h2 class="text-center">Tidak Ada Pembelian!</h2>`;
                loader.style.display = "none";
            }
            else {
                    ...
                    <div class="card" style="width: 17rem; height: 30 !important;">
                        ...
                        <div class="card-body">
                            ...
                            <div class="d-flex justify-content-between">
                                <p class="" style="font-size: 16px;">Status : ${doc.data().itemstatus} <br /> Nomor Resi : ${doc.data().itemresi}</p>
                            </div>
                        </div>
                    </div>
                    </div>`
